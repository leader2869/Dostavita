  ...
  return (
    <>
      <div className="bg-gray-50 rounded-lg shadow p-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Чаты с заказчиками</h2>
...
function OrderChatItem({ order, driverUserId, onOpenChat }: OrderChatItemProps) {
  ...
  return (
    <button
      onClick={onOpenChat}
      className="w-full bg-gray-100 hover:bg-gray-100 rounded-lg p-4 transition text-left"
    >
      <div className="flex items-center justify-between">
        ...
        {unreadCount > 0 && (
          <span className={`bg-red-500 text-gray-900 text-xs font-bold rounded-full flex items-center justify-center ${
            unreadCount > 9 ? 'px-2 min-w-[1.5rem]' : 'w-6 h-6'
          }`}>
            {unreadCount > 10 ? unreadCount : unreadCount >= 10 ? 10 : unreadCount}
          </span>
        )}
        <svg className="w-5 h-5 text-gray-600 ml-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
