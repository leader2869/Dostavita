-- Миграция 097: Исправление неоднозначной ссылки на колонку id в функции get_organization_receivables
...
COMMENT ON FUNCTION public.get_organization_receivables(UUID, TIMESTAMPTZ, TIMESTAMPTZ) IS 
  'Возвращает дебиторку (неоплаченные заказы) для организации. Использует organization_id для прямого фильтрования. Исправлена неоднозначная ссылка на колонку id.';
