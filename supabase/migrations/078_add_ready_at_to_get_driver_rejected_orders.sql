-- Миграция 078: Добавление поля ready_at в функцию get_driver_rejected_orders
-- Это поле нужно для отображения времени готовности заказа в доступных заказах
...
COMMENT ON FUNCTION public.get_driver_rejected_orders(UUID) IS 
  'Возвращает заказы, от которых водитель отказался, но которые все еще активны (status = searching_courier). Включает поле ready_at для отображения времени готовности заказа.';
