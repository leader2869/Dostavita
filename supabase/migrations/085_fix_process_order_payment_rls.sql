-- Миграция 085: Исправление RLS для функции process_order_payment
-- Проблема: функция с SECURITY DEFINER не может обновлять баланс из-за RLS
-- Решение: используем SET LOCAL для временного отключения RLS в функции

-- Удаляем старую функцию
DROP FUNCTION IF EXISTS public.process_order_payment(UUID, BOOLEAN);

-- Создаем функцию заново с правильной обработкой RLS
...
BEGIN
  ...
  IF payment_status THEN
    ...
    -- Временно отключаем RLS для операций с балансом и транзакциями
    -- Это критически важно для работы функции с SECURITY DEFINER
    ...
    BEGIN
      ...
      -- Проверяем, что транзакция создана (GET DIAGNOSTICS не работает для INSERT)
      -- Вместо этого просто проверяем, что не было ошибки
